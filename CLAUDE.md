...
**Logistics tracker** (`LogisticsTracker.jsx`, page `/logistics`) — standalone (not linked to inventory) shipment tracker with 24 fields. Status uses an 11-stage workflow: Order Received → Source Chosen → Invoice Issued → Invoice Passed to Finance Dep → Invoice Paid → In Lead Time → Ready To Pickup → Picked Up / On The Way → Arrived to Armenia → In Custom Clearance → Delivered / Received. Several fields are dropdowns: Payment Method (Invoice, PayPal), Currency (USD, EUR, AMD), Invoice Status, Source of Procurement, Delivery Term, Transportation Payment, Custom Clearance Status, Expertise Fee Payment Status. ETA field turns red when overdue. **Important**: status and other dropdowns use native `<select>` elements — do NOT use absolute-positioned custom popovers inside `overflow-auto` table containers, as they get clipped and only the first item appears. Column widths key: `logistics-column-widths-v2`.
